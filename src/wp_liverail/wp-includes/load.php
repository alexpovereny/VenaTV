...
function require_lr_api() {
    global $lrapi, $ENTITY_ID;

    require_once( ABSPATH . WPINC . '/liverailapi.php' );
    //echo (date("l dS of F Y h:I:s A"));
    // echo '<br> $lrapi <br>';
    // var_dump($lrapi);
    if (isset($lrapi))
    ...
    $lrapi = new LiveRailApiGlobal(LRAPI_URL);
    $api_params = $lrapi->login(LRAPI_USER, LRAPI_PASSWORD);
    //$get_xml_doc = $lrapi->getLastApiXmlDoc();
    //var_dump($get_xml_doc);
    //$ENTITY_ID = $get_xml_doc->auth->entity->entity_id;
    // $lrapi = new LiveRailApiGlobal('http://api4.int.liverail.com');
    // $lrapi->login('alex.povereny@gmail.com', 'alexvena2014');
}
