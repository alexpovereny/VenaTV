...
switch ($wp_list_table->current_action()) {
        ...
        foreach ($userids as $id) {
            ...
            switch ($_REQUEST['delete_option']) {
                case 'delete':
                    ...
                    $del_user_id = delete_liverail_user($id);
                    $user_delete_res = $lrapi->callApi("/user/delete", array('user_id' => $del_user_id));
                    wp_delete_user($id);
                    break;
                case 'reassign':
                    $mvv_global_entity_id = get_option('mvv_global_entity_id');
                    ...
                    <fieldset><p><legend><?php echo _n('What should be done with posts owned by this user?', 'What should be done with posts owned by these users?', $go_delete); ?></legend></p>
                        <ul style="list-style:none;">
                            ...
                            <li><input type="radio" id="delete_option1" name="delete_option" value="reassign" />
                                <?php
                                echo '<label for="delete_option1">' . __('Attribute all posts to:') . '</label> ';
    ...
    default:
        ...
        <?php
        $users = get_users('user_entity_id=27763');
        //echo '<br>--- users--- <br>';
        ...
        $get_all_entity = '';
        if ($get_all_entity == '') {
            $get_all_entity = $wpdb->get_results("SELECT entity_id, organization, status FROM wp_entity"); // WHERE status='active'
            $get_entity_id = $_GET['entity_id'];
        }
        ...
        if ($_POST['entity_id']) {
            ...
            if ($get_entity_res != FALSE) {
                $mvv_entity_id = (int) $get_xml_doc->auth->entity->entity_id;
                update_option('mvv_global_entity_id', $mvv_entity_id);
                $messages[] = '<div id="message" class="updated"><p>' . __('Change entity ' . $mvv_entity_id) . '</p></div>';
                if ($get_xml_doc->warnings) {
                    $error_msg_ = (string) $get_xml_doc->error->message;
                    $error_msg_field = (string) $get_xml_doc->error->field;
                    $errors = new WP_Error();
                    $errors->add($error_msg_field, __('<strong>Warning</strong>: ' . (string) $get_xml_doc->warnings->warning->message));
                    //echo (string) $get_xml_doc->warnings->warning->message;
                }
            } else {
                $error_msg_ = (string) $get_xml_doc->error->message;
                $error_msg_field = (string) $get_xml_doc->error->field;
        ...
        <div class="wrap">
            ...
            <form method="post" action="">
                <select name="entity_id" id="entity_id">
                    <?php
                    $r .="<option value=''>--- Select entity ---</option>";
                    foreach ($get_all_entity as $key => $value_entity) {
                        $name = $value_entity->organization;
                        $entity_id = $value_entity->entity_id;
                        $status = $value_entity->status;

                        if (get_option('mvv_global_entity_id') == $value_entity->entity_id)
                            $p = "\n\t<option selected='selected' value='" . esc_attr($value_entity->entity_id) . "'> $name / $entity_id / $status</option>";
                        else
                            $r .= "\n\t<option value='" . esc_attr($value_entity->entity_id) . "' style='background-color: red; color: white;'> $name / $entity_id / $status</option>";
                    }
                    echo $p . $r;
                    ?>
                </select>
                <input type="hidden" name="change_entity_id" id="change_entity_id" value="true"/>
                <input type="submit" name="change_entity" id="change_entity" class="button" value="Change entity">
            </form>
