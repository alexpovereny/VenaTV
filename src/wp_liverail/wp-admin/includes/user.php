 ...
 */
function edit_user($user_id = 0, $liverail_user_id = 0, $liverail_entity_id = 0) {
    ...
    if ($liverail_user_id)
        $user->liverail_user_id = $liverail_user_id;

    if ($liverail_entity_id)
        $user->liverail_entity_id = $liverail_entity_id;
...
function delete_liverail_user($id) {
    global $wpdb;
    $user_params = $wpdb->get_results("SELECT liverail_user_id FROM wp_users WHERE ID = '" . $id."'");  
    $user_id = $user_params['0']->liverail_user_id;
    return $user_id;
}
