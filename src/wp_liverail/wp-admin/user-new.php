...
global $lrapi;
$mvv_liverail_options = get_option(MVVLIVERAIL . 'options');
/*
  echo '<br> funct delete_liverail_user <br>';
  $del_user_id = delete_liverail_user('13');
  $user_delete_res = $liverail_api->callApi("/user/delete", array('user_id' => $del_user_id));
  echo '<br>--- $user_delete_res ---<br>';
...
%4$s');
    ...
    //echo '<br> elseif  !!! $_REQUEST <br>';
    //var_dump($_REQUEST);
    //{ ["action"]=> string(10) "createuser" ["_wpnonce_create-user"]=> string(10) "d09a529c7d" ["_wp_http_referer"]=> string(54) "/wp_liverail_git/src/wp_liverail/wp-admin/user-new.php" ["user_login"]=> string(5) "test3" ["email"]=> string(15) "test3@gmail.com" ["first_name"]=> string(5) "test3" ["last_name"]=> string(5) "test3" ["url"]=> string(5) "test3" ["pass1"]=> string(5) "test3" ["pass2"]=> string(5) "test3" ["role"]=> string(10) "subscriber" ["createuser"]=> string(13) "Add New User " }
    ...
    // method LiveRail API !!!
    //User's account role on the current entity;
    $user_password = "cust-rw"; // customer read-write
    if ($_REQUEST['role'] == 'administrator' || $_REQUEST['role'] == 'network') {
        $user_password = "admin";
    }
    $array_user_params = array(
        ...
        "role" => $user_password);
    //Добавление пользователя в LiveRail API
    // $user_add_res = $liverail_api->callApi("/user/add", $array_user_params);
    $mvv_global_entity_id = get_option('mvv_global_entity_id');
    $get_entity_res = $lrapi->callApi("/set/entity", array(
        "entity_id" => $mvv_global_entity_id
    ));
    $user_add_res = $lrapi->callApi("/user/add", $array_user_params);
    // Если через LiveRail API был добавлен пользователь удачно
    if ($user_add_res == TRUE) {
        $get_json_doc = $lrapi->getLastApiJSONDoc();
        //echo '<br>____' . get_option('mvv_global_entity_id');
        //echo '<br>--- $get_json_doc!!! ---<br>';
        //var_dump($get_json_doc);
        $liverail_user_id = $get_json_doc->user_id;
        $liverail_entity_id = get_option('mvv_global_entity_id'); //$get_json_doc->auth->entity->entity_id;

        if (!is_multisite()) {
            $user_id = edit_user(0, $liverail_user_id, $liverail_entity_id);
        ...
        //Ошибка при добавлении пользователя в LiveRail 
        ...
        $errors = new WP_Error();
        $errors->add($error_msg_field, __('<strong>ERROR</strong>: ' . $error_msg_));
    }
}
                    ...
                    <td><select name="role" id="role">
                            ...
                            if (current_user_can('network')) {
                                ...
                                foreach ($editable_roles as $role => $details) {
                                    ...
                                    if ($role == 'network' || $role == 'publisher' || $role == 'advertiser')
